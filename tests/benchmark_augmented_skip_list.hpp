...
int *GetBatchIndices(int num_elements, int batch_size, int *perm,
                     const std::string &batch_type) {
  int *batch_indices{new_array_no_init<int>(batch_size)};
  if (batch_type == "random") {

    parlay::parallel_for(0, batch_size,
                         [&](size_t i) { batch_indices[i] = perm[i]; });
...
void RunBenchmark(Element *elements, const BenchmarkParameters &parameters) {
  ...
  ElementPPair *construct_joins{
      new_array_no_init<ElementPPair>(num_elements - 1)};
  Element **destruct_splits{new_array_no_init<Element *>(num_elements - 1)};
  ConstructJoinsAndSplitsFromIndices(perm, elements, num_elements - 1,
                                     construct_joins, destruct_splits);

  int *batch_indices{
      GetBatchIndices(num_elements, batch_size, perm, parameters.batch_type)};
  ElementPPair *batch_joins{new_array_no_init<ElementPPair>(batch_size)};
  Element **batch_splits{new_array_no_init<Element *>(batch_size)};
  ConstructJoinsAndSplitsFromIndices(batch_indices, elements, batch_size,
                                     batch_joins, batch_splits);
  ...
  delete_array(construct_joins, num_elements - 1);
  delete_array(destruct_splits, num_elements - 1);
  delete_array(batch_joins, batch_size);
  delete_array(batch_splits, batch_size);
}
